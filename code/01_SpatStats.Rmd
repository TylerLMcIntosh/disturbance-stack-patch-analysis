...
aoiL3Names <- c("Southern Rockies") #The names of the EPA Level 3 ecoregions of interest, may be any length
...
#Set outdirectory
if(computing == "local") {
  outDirT <- here::here("data", "derived", "metric_tables")
  if (!dir.exists(outDirT)){
    dir.create(outDirT)
  }
} else if(computing == "cyverse") {
  outDirT <- "~/data-store/data/iplant/home/shared/earthlab/macrosystems/disturbance-stack-patch-analysis/metric_tables"
  if (!dir.exists(outDirT)){
    dir.create(outDirT)
  }
}
...
if(computing == "local") {
  ...
  insectStack <- terra::rast("~/data-store/data/iplant/home/shared/earthlab/macrosystems/disturbance-stack-patch-analysis/derived/insect_dist_stack_west.tif")
...
#This function requires a set name (setNm) as a string, which clarifies what the input raster represents (e.g. "Fire")
calculate.raster.metrics.for.aoi.set <- function(raster, aoiSet, setNm) {
...
# #write out
# classL4 %>%
#    purrr::map2(.f = write.land.metric.csv.w.name, .y = names(.), dir = outDirT, set = "FireInsect")
# # Read in
# z <- read.set.csvs.as.one("FireInsect")
...
testOut <- calculate.raster.metrics.for.aoi.set(fireInsects, aoiL4Interest[1:2,], setNm = "FireInsects2")
