...
#Set dynamicRoot to set data location
if(computing == "cyverse") {
  dynamicRoot <- "../iplant/home/shared/earthlab/macrosystems/disturbance-stack-patch-analysis"
} else {
  dynamicRoot <- ""
}

# A function to test a function that has been set up to run in either sequence or parallel using the futureverse, depending on what plan() has been set. For use in troubleshooting.
...
```{r, echo = FALSE}
#Load data
fileLocation <- here::here(dynamicRoot, 'data', 'nathan_outputs')
...
testFireStack <- terra::rast(here::here('data', 'derived', 'fire_dist_stack_west.tif'))
...
toc()
fireTestList <- careful.clip.set(testFireStack, aoiL4Interest[1:2,], "us_l4l3name")
...
add.data.to.metric.df <- function(df, nm) {
  #Add year from layer data
  if(length(unique(df$layer)) == 1) { #If single-layer raster was used, it is not a time series
  df <- df |>
    dplyr::mutate(year = NA)
  } else {
    df <- df |>
      dplyr::mutate(year = layer + 1998) #NOTE: this is hard-coded in, not pulled from raster!
  }
...
calculate.class.level.metrics <- function(landList) {
  if(is(future::plan(), "sequential")) {
      ...
      out <- landList |>
        ...
        furrr::future_map(parallel.calculate.lsm, #use parallelized version that unwraps first
                   what = c(
                             'lsm_c_ed', #patch area-weighted edge density
                             'lsm_c_ca', #total class area
                             'lsm_c_ai',
  ...
  out <- out |>
    purrr::map(dplyr::left_join, y = lsm_abbreviations_names, #Add full metric names
                         by = c("metric", "level")) %>%
    purrr::map2(.f = add.data.to.metric.df, .y = names(.)) #Add more data using add.data.to.metric.df function
  return(out)
}
...
test.future.function("calculate.class.level.metrics", fireInsectTestList)
