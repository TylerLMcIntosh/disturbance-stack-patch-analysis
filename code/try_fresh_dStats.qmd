...
if (!"l3InterestNames.csv" %in% list.files(devDir, full.names = FALSE)) {
  ...
  write.csv(epaL3NamesInterest,
            ...
            row.names=F)
  if(computing == "cyverse") {
    system("cp /home/jovyan/data-store/disturbance-stack-patch-analysis/derived/l3InterestNames.csv ~/data-store/data/iplant/home/shared/earthlab/macrosystems/disturbance-stack-patch-analysis/data/derived ")
  }
...
calculate.metrics.for.raster.aoi.set <- function(rasterFileName, aoiSet, setNm, freqsOnly) { 
  ...
  print(glue::glue('Starting parallel processing for {lyrNm}'))
  allDats <- vs |> furrr::future_map(~calculate.raster.metrics(raster = rWrap, aoi = .x, setNm = setNm, freqsOnly = freqsOnly), .options = furrr_options(seed = TRUE))
...
calculate.raster.metrics <- function(raster, aoi, setNm, freqsOnly) {
...
# TEST ON SINGLE L3 ECOREGION
tic()
calculate.raster.metrics(raster = r, aoi = testSR, setNm = "FIRSTTEST", freqsOnly = TRUE) 
toc()
#67 sec f only
#23 sec on cyverse

tic()
calculate.raster.metrics(raster = r, aoi = testSR[1:100,], setNm = "FIRSTTEST", freqsOnly = TRUE) 
toc()
#9.17 sec
#3 sec on cyverse

tic()
calculate.raster.metrics(raster = r, aoi = testSR, setNm = "FIRSTTESTALL", freqsOnly = FALSE) 
toc()
# 1416 sec both - 0s and 1s
# 530 sec both - 1s only


tic()
calculate.raster.metrics(raster = r, aoi = testSR[1:100,], setNm = "FIRSTTEST", freqsOnly = FALSE) 
toc()
...
tic()
calculate.metrics.for.raster.aoi.set(rasterFileName = beetle5yrF[1], aoiSet = epaL4Interest, setNm = "FIRSTTESTPAR", freqsOnly = TRUE) 
toc()
#236.8 sec - 4 min
...
tic()
calculate.metrics.for.raster.aoi.set(rasterFileName = beetle5yrF[1], aoiSet = epaL4Interest, setNm = "FIRSTTESTPAR", freqsOnly = FALSE) 
toc()
...
#Single 5 yr
tic()
beetle5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "beetle5yr", freqsOnly = TRUE))
toc()

fire5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "fire5yr", freqsOnly = TRUE))

drought5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "drought5yr", freqsOnly = TRUE))


#Double 5yr
fireBeetle5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "fireBeetle5yr", freqsOnly = TRUE))

fireDrought5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "fireDrought5yr", freqsOnly = TRUE))

droughtBeetle5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "droughtBeetle5yr", freqsOnly = TRUE))

#Triple 5yr
fireBeetleDrought5yrF |> purrr::map(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "fireBeetleDrought5yr", freqsOnly = TRUE))
