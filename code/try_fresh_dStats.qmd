...
calculate.metrics.for.raster.aoi.set <- function(rasterFileName, aoiSet, setNm, freqsOnly, verbose, metricMergeThresh, metricIgnore) { 
  ...
  allDats <- vs |> furrr::future_map(~calculate.raster.metrics(raster = rWrap, aoi = .x, setNm = setNm, freqsOnly = freqsOnly, verbose = verbose, metricMergeThresh = metricMergeThresh, metricIgnore = metricIgnore), .options = furrr_options(seed = TRUE))
...
calculate.raster.metrics <- function(raster, aoi, setNm, freqsOnly, verbose, metricMergeThresh = NA, metricIgnore = NA) {
...
#Double 5yr
tic("fireBeetle5yr")
fireBeetle5yrF |> purrr::walk(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "fireBeetle5yr", freqsOnly = FALSE, verbose = FALSE, metricMergeThresh = 2))
toc()
...
droughtBeetle5yrF |> purrr::walk(~calculate.metrics.for.raster.aoi.set(rasterFileName = .x, aoiSet = epaL4Interest, setNm = "droughtBeetle5yr", freqsOnly = FALSE, verbose = FALSE, metricMergeThresh = 2))
