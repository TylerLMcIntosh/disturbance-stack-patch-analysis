...
calculate.raster.metrics <- function(raster, aoi, setNm, freqsOnly = TRUE) {
  ...
  readr::write_csv(fs, here::here(outDir, glue::glue("freqs_{setNm}_{unique(aoi$US_L3CODE)}.csv")))
  if(!freqsOnly) readr::write_csv(ms, here::here(outDir, glue::glue("metrics_{setNm}_{unique(aoi$US_L3CODE)}.csv")))
...
tic()
calculate.raster.metrics(raster = r, aoi = testSR, setNm = "FIRSTTEST", freqsOnly = FALSE) 
toc()
