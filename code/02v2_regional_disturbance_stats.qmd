...
Tyler L. McIntosh CU Boulder CIRES Earth Lab Last updated: 1/24/24
...
#Set here location
here::i_am("code/02v2_regional_disturbance_stats.qmd")
...
calculate.metrics.for.raster.aoi.set <- function(rasterFileName, aoiSet, setNm) { 
  ...
  allDats <- c('cat', 'dog') |> furrr::future_map(~calculate.raster.metrics(rWrap = "rWrap", aoi = .x) )#, .options = furrr_options(seed = TRUE))
  
  
  # #Run calculate raster metrics function in parallel
  # print('Starting parallel processing')
  # allDats <- vs |> furrr::future_map(~calculate.raster.metrics(rWrap = rWrap, aoi = .x) )#, .options = furrr_options(seed = TRUE))
  # 
...
calculate.raster.metrics <- function(rWrap, aoi) {
  
  # #Clip the raster to the aoi sent to node
  # clippedR <- careful.clip(raster = terra::unwrap(rWrap), vector = unwrap(aoi), mask = TRUE)
  # 
  # #Collect frequencies 
...
#local - uses all local memory but continues processing, 544 seconds, success
